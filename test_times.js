...
async function test(model, date) {
  ...
  if (data === true) {
    console.log(`[AVAILABLE] Model: ${model}, Date: ${date} =>`, data);
  }
}

async function run() {
  for (let hour = 0; hour < 48; hour++) {
     const t = new Date(Date.UTC(2026, 3, 19, 0, 0, 0));
     t.setUTCHours(t.getUTCHours() + hour);
     const dateStr = t.toISOString().split('T')[0] + ' ' + t.toISOString().split('T')[1].substring(0,8);
     await test("Saga", dateStr);
  }
}
run();
